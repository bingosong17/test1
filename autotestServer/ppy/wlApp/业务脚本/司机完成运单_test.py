...
from wlApp.api.获取未完成的运单列表_test import TestCase获取未完成的运单列表 as 获取未完成的运单列表
...
class TestCase取送货(HttpRunner):
    ...
    config = Config("testcase description").verify(False).base_url("${testEnv($business)}").variables(
        **{
            "business": "",
            "transOrderId": 1141027651190781890,
            "phoneNumber": "15267049232",
            "token": "${getSession($business,$phoneNumber)}",
            "userId": "${getWlAppUserId($phoneNumber)}",
            "orderNo": "2020092814435811614637",
        }
    )
    teststeps = [
        Step(
            RunTestCase("获取未完成的运单列表").call(获取未完成的运单列表)
        ),
        Step(
        ...
        Step(
            RunTestCase("获取配送单商品详情").call(获取配送单商品详情)
        ),
        Step(
