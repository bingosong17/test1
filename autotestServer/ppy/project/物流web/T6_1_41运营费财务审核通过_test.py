...
from yyht.物流中心.运力管理.api.运营费财务审核通过_test import TestCase运营费财务审核通过 as 运营费财务审核通过
...
class TestCase运营费业务审核通过(HttpRunner):
    ...
    teststeps = [
        ...
        Step(
            ...
            ).call(提交运营费).teardown_hook("${sleep_N_secs(1)}")
        ...
        Step(
            RunTestCase("财务审核通过").with_variables(
                **{
                    "ids": ["${exportValue($pageData,id,description=strType$descriptions)}"]
                }

            ).call(运营费财务审核通过).teardown_hook("${sleep_N_secs(1)}")
        ),
        Step(
            RunTestCase("运费查询").with_variables(
                **{
                ...
                .teardown_hook("${assert_equal($status,20)}")
        ),
    ]
