...
from yyht.物流中心.CTMS设置.api.删除商品配置_test import TestCase删除商品配置 as 删除商品配置
...
class TestCase增加商品配置(HttpRunner):
    ...
    teststeps = [
        ...
        Step(
            ...
            ).call(商品配置查询).export(*["id", "goodsName"])
                .teardown_hook("${assert_equal($goodsName,$goodsNames)}")

        ),
        Step(
            RunTestCase("删除商品配置").with_variables(
                **{
                    "id": "$id"
                }
        ...
        Step(
            RunTestCase("商品配置查询").with_variables(
                **{
                ...
                .teardown_hook("${assert_not_equal($goodsName,$goodsNames)}")
