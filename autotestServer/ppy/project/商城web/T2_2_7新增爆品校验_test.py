...
class TestCase新增爆品校验(HttpRunner):
    ...
    teststeps = [
        ...
        Step(
            ...
            ).call(爆品列表).export(*["goodsId", "id", "pageData"])
            .teardown_hook("${sleep_N_secs(4)}")
            .teardown_hook("${assert_equal($goodsId,$goodsIdTo227)}")
        ),
        Step(
            RunTestCase("删除爆品")
            .with_variables(
                **{
                    "surpriseId": "${exportValue($pageData,id,goodsId=$goodsIdTo227)}"}
            )
            .call(删除爆品)
